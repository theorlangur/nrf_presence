...
ZEPHYR_ELF=$(find "${BUILD_DIR:-build_ezurio_llvm}/presence_multi_v2" -name zephyr.elf -print -quit)
...
python3 breadcrumbs/scripts/jlink_extract_breadcrumbs.py "$PART_ADDR" breadcrumbs/bread.bin --max-payload "$PART_SIZE" --elf "$ZEPHYR_ELF"
